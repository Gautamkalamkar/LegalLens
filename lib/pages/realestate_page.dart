...
class _RealestatePageState extends State<RealestatePage> {
  ...
  List<Map<dynamic, dynamic>> realEstates = [];
            ...
            return DocumentView(
                ...
                onDelete: () {
                  _hiveService.deletekey(
                      index, realEstates, box, 'realEstates');
                  setState(() {});
                });
          },
        ),
        ...
        child: FittedBox(
          child: FloatingActionButton(
            onPressed: () async {
              ...
              if (!_hiveService.isDuplicate(file.name.trim(), realEstates)) {
                realEstates.add({
                  'name': file.name.trim(),
                  'path': file.path,
                  'text': text
                });
                box.put('realEstates', realEstates);
