...
class _CorporatePageState extends State<CorporatePage> {
  ...
  List<Map<dynamic, dynamic>> corporates = [];
      ...
      body: Padding(
        ...
        child: ListView.builder(
          ...
          itemBuilder: (context, index) {
            final corporate = corporates[index];
            return DocumentView(
                name: corporate['name'],
                path: corporate['path'],
                docType: 'corporates',
                onDelete: () {
                  _hiveService.deletekey(index, corporates, box, 'corporates');
                  setState(() {});
                });
          },
        ),
        ...
        child: FittedBox(
          child: FloatingActionButton(
            onPressed: () async {
              ...
              if (!_hiveService.isDuplicate(file.name.trim(), corporates)) {
                corporates.add({
                  'name': file.name.trim(),
                  'path': file.path,
                  'text': text
                });
                box.put('corporates', corporates);
