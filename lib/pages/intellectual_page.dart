...
class _IntellectualPageState extends State<IntellectualPage> {
  ...
  List<Map<dynamic, dynamic>> intellectuals = [];
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: ListView.builder(
          itemCount: intellectuals.length,
          itemBuilder: (context, index) {
            final intellectual = intellectuals[index];
            return DocumentView(
                name: intellectual['name'],
                path: intellectual['path'],
                docType: 'intellectuals',
                onDelete: () {
                  _hiveService.deletekey(
                      index, intellectuals, box, 'intellectuals');
                  setState(() {});
                });
          },
        ),
        ...
        child: FittedBox(
          child: FloatingActionButton(
            onPressed: () async {
              ...
              if (!_hiveService.isDuplicate(file.name.trim(), intellectuals)) {
                intellectuals.add({
                  'name': file.name.trim(),
                  'path': file.path,
                  'text': text
                });
                box.put('intellectuals', intellectuals);
